package com.example.multiable.bottomtoup.CustomView;
...
import com.example.multiable.bottomtoup.R;
...
public class MainUI extends FrameLayout {
    ...
    public MainUI(Context context, AttributeSet attrs) {
        ...
        this.context = context;
        //初始化自定义的属性
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.MainUI);
        int n = a.getIndexCount();
        for (int i = 0; i < n; i++) {
            int attr = a.getIndex(i);
            switch (attr) {
                case R.styleable.MainUI_front_color:
                    middleColor = a.getColor(attr, context.getResources().getColor(R.color.white));
                    break;
                case R.styleable.MainUI_back_color:
